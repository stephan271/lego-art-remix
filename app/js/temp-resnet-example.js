async function runExample() {
    ...
    await session.loadModel("models/resnet50_8.onnx");
...
function imagenetClassesTopK(classProbabilities, k) {
    console.log("hello from imagenetClassesTopK");
...
function printMatches(data) {
    console.log("hello from printMatches");
    let outputClasses = [];
    ...
    for (let i of [0, 1, 2, 3, 4]) {
        results.push(
            `${outputClasses[i].name}: ${Math.round(
                100 * outputClasses[i].probability
            )}%`
        );
    }
